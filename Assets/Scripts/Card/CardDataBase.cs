...
1100,吸血,进阶,2,1,3,当你造成伤害后，恢复伤害值[数值]倍的生命值，持续[持续时间]回合,打完身上没一滴血是自己的
1101,加速,进阶,0,""1,2"",3,将游戏加速0.1x，并每回合造成[数值1]点伤害，此伤害每回合×[数值2]，持续[持续时间]回合,我说，我已启动，你耳朵懂吗
1102,延续,进阶,1,1,0,使所有卡牌和效果的持续时间延长[数值]回合,老板，能不能再加个钟
1103,超频,进阶,1,2,0,使所有卡牌的魔力消耗和数值×[数值],超频提升角色和武器战力的核心机制
1104,结算,进阶,1,1,0,立即结算[数值]次双方身上的所有效果,你的月付账单已生成
1105,急救,进阶,0,""1,3"",0,立即获得[数值1]点魔力并恢复[数值2]点生命,我觉得我还可以再抢救一下
1106,闪击,进阶,0,5,0,立即造成[数值]点伤害,你甚至没看清刀光，一切就结束了
1110,重奏,进阶,0,0,0,本牌在你手牌中时，当你使用一张卡牌后，变形为它的复制,人类的本质是复读机
1200,七罪,原罪,7,""6,1"",7,获得全部[数值1]张“七宗罪”卡牌，此后每回合随机获得[数值2]张，持续[持续时间]回合,二手全新，拆开概不退换
1201,暴怒,原罪,0,""7,7"",0,造成[数值1]点伤害，有[数值2]%概率受到同等伤害,怒火烧尽敌人，同时燃尽自我
...
1205,懒惰,原罪,0,7,0,将游戏减速0.1x，消耗至多[数值]点法力值并抽等量张牌，结束此回合,今天有点累了，明天再更新吧
1207,暴食,原罪,0,""7,7"",0,吃掉至多[数值1]张手牌，每吃掉一张，恢复[数值2]点生命,拜拜甜甜圈，珍珠奶茶大盘鸡
1300,种子,随机,0,1,0,重置随机数种子，随机获得[数值]张牌,何意味
...
1304,赌徒,随机,1,1,0,""[数值/1+数值]概率获得一张""""随机""""牌并再次触发此卡效果"",一无所有？或者，赢下所有
1305,轮盘,随机,1,""1,36"",6,5/6概率对自己造成[数值1]伤害，1/6概率对敌人造成[数值2]点伤害，持续[持续时间]回合,咔哒。咔哒。咔哒。砰
1400,苦修,鲜血,1,1,5,你对自己造成伤害时，获得[数值]点魔力，持续[持续时间]回合,人生是一场修行
1401,献祭,鲜血,1,""1,2"",5,每回合对双方造成[数值1]点伤害，本局对战内“献祭”伤害永久×[数值2]，持续[持续时间]回合,复活吧，我的爱人
1402,卖血,鲜血,0,""2,1"",5,对自己造成[数值1]点伤害，抽[数值2]张牌，持续[持续时间]回合,许三观卖血记读后感1000字（通用8篇）-- 作文人网
1403,反伤,鲜血,1,1,5,受伤后，对敌造成伤害值[数值]倍的伤害，持续[持续时间]回合,用毒蛇的毒毒毒蛇，毒蛇会不会被毒死
1404,血契,鲜血,2,1,3,当你恢复生命后，对敌人造成恢复量[数值]倍的伤害，持续[持续时间]回合,一起成为马猴烧酒吧
...
1802,传送,科技,1,1,3,将你的所有手牌传送到下[数值]局,要是能重来，我要选李白";
    ...
    private static void ParseCsvContent(string csvContent)
    {
        using (StringReader reader = new(csvContent))
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (!TryParseValues(fields[4], out List<ulong> parsedValues))
                {
                ...
                CardData data = new()
                {
                    ...
                    series = fields[2],
                    cost = costValue,
                    value = parsedValues.Count > 0 ? parsedValues[0] : 0UL,
                    values = parsedValues,
    ...
    private static bool TryParseValues(string rawValue, out List<ulong> values)
    {
        ...
        for (int i = 0; i < parts.Length; i++)
        {
            ...
            if (!ulong.TryParse(trimmed, out ulong parsed))
            {
                values.Clear();
                return false;
            }
